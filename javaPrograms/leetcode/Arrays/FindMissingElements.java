...
class FindMissingElements {
    private static List<Integer> findMissingElements(int[] nums) {
        ...
        int i = nums[0], j = 0, n = nums.length;
        for (; i <= nums[n - 1] && j < n; ++i) {
            if (i != nums[j]) list.add(i);
            else ++j;
        }
        return list;
    }
    public static void main(String[] args) {
        Log.info(findMissingElements(new int[] {1,4,2,5}));
        Log.info(findMissingElements(new int[] {7,8,6,9}));
        Log.info(findMissingElements(new int[] {5,1}));
    }
}
