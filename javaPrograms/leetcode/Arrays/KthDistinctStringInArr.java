...
class KthDistinctStringInArr {
    private static String kthDistinct(String[] arr, int k) {
        ...
        for (String s : arr) mpp.put(s, mpp.getOrDefault(s, 0) + 1);
        for (String s : arr) {
            if (mpp.get(s) == 1) {
                k--;
                if (k == 0) return s;
    ...
    public static void main(String[]args) {
        log.info(kthDistinct(new String[]{"d","b","c","b","c","a"}, 2));
        log.info(kthDistinct(new String[]{"aaa","aa","a"}, 3));
        log.info(kthDistinct(new String[]{"a","b","a"}, 3));
    }
}
