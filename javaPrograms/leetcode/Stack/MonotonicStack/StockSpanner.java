 ...
 */
class StockSpanner {
    private Deque<HashMap<Integer, Integer>> st;
    private int index = -1;
    public StockSpanner() {
        index = -1;
        st = new ArrayDeque<>();
    ...
    public int next(int price) {
        index++;
        while(!st.isEmpty()) {
            HashMap<Integer, Integer> top = st.peek();
            int topPrice = top.values().iterator().next();
            if(topPrice <= price) st.pop();
            else break;
        }
        int ans = index - (st.isEmpty() ? -1 : 
            st.peek().keySet().iterator().next());
        st.push(new HashMap<>(Map.of(index, price)));
        return ans;
    }
}
