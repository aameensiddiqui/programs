...
class CountBinarySubstrings {
    private static int countBinarySubstrings(String s) {
        int prv = 0, cur = 1, n = s.length(), ans = 0;
        for (int i = 1; i < n; ++i) {
            if (s.charAt(i - 1) != s.charAt(i)) {
                ans += Math.min(prv, cur);
                prv = cur;
                cur = 1;
            } else cur++;
        }
        return ans + Math.min(prv, cur);
    }
    public static void main(String[]args) {
        Log.info(countBinarySubstrings("00110011"));
        Log.info(countBinarySubstrings("10101"));
    }
}
