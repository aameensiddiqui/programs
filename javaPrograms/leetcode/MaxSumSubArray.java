 ...
 * Kadane's Algorithm
 *
 * 53. Maximum Subarray Sum in an Array
 ...
 * 
 * Here, we will iterate the given array with a single loop and while iterating we will add
 * the elements in a sum variable. Now, if at any point the sum becomes less than 0,
 * we will set the sum as 0 as we are not going to consider any subarray with a negative sum.
 ...
 * If at any point the sum becomes negative we will set the sum to 0 as we are not going to
 * consider it as a part of our answer.
 * Note: In some cases, the question might say to consider the sum of the empty subarray
 * while solving this problem. So, in these cases, before returning the answer we will compare
 * the maximum subarray sum calculated with 0(i.e. The sum of an empty subarray is 0).
 ...
 * 
 * If we carefully observe our algorithm, we can notice that the subarray always starts at the
 * particular index where the sum variable is equal to 0, and at the ending index, the sum always
 * crosses the previous maximum sum(i.e. maxi).
 * 
 * So, we will keep a track of the starting index inside the loop using a start variable.
 * We will take two variables ansStart and ansEnd initialized with -1. And when the sum crosses
 * the maximum sum, we will set ansStart to the start variable and ansEnd to the current index i.e. i.
 ...
 * The first loops(say i and j) will iterate over every possible starting index and
 * ending index of a subarray. Basically, in each iteration, the subarray range will
 * be from index i to index j. Using another loop we will get the sum of the elements
 * of the subarray [i..j]. Among all values of the sum calculated, we will consider the maximum one.
 * 
 * Approach:
 * The steps are as follows:
 * 
 ...
 * For every subarray starting from the index i,
