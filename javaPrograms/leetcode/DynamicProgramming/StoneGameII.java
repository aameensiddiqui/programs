/**
   ...
   Explanation:
   If Alice takes one pile at the beginning, Bob takes two piles, then Alice takes 2 piles
   again. Alice can get 2 + 4 + 4 = 10 stones in total.
    ...
    private static int dfs(int alice, int i, int M, int[] piles) {
        ...
        for (int X = 1; X <= 2 * M; ++X) {
            ...
            if (alice == 1)
                res = Math.max(
                    res,
                    total + dfs(0, i + X, Math.max(M, X), piles)
                );
            else
                res = Math.min(
                    res,
                    dfs(1, i + X, Math.max(M, X), piles)
                );
        }
