...
class UniqueEmails {
    ...
    public static void main(String[]args) {
        Log.info(numUniqueEmails(new String[]{"test.email+alex@leetcode.com",
                                              "test.e.mail+bob.cathy@leetcode.com",
                                              "testemail+david@lee.tcode.com"}));
        Log.info(numUniqueEmails(new String[]{"a@leetcode.com",
                                              "b@leetcode.com",
                                              "c@leetcode.com"}));
    }
}
    ...
    private static int numUniqueEmails(String[] emails) {
        ...
        for (String email : emails) {
            ...
            while (i < email.length() && email.charAt(i) != '@' && email.charAt(i) != '+') {
                if (email.charAt(i) != '.') local.append(email.charAt(i));
                i++;
            }
            while (i < email.length() && email.charAt(i) != '@') i++;
            String domain = email.substring(i + 1);
            set.add(String.valueOf(local) + "@" + domain);
        }
        return set.size();
    }
*/
