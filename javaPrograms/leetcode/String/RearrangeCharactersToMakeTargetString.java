...
class RearrangeCharactersToMakeTargetString {
    private static int rearrangeCharacters(String s, String target) {
        ...
        for (char ch : target.toCharArray()) {
            freqT[ch - 'a']++;
            if (freqS[ch - 'a'] == 0) return 0;
        }
        int targetCopies = Integer.MAX_VALUE;
        for (char ch : s.toCharArray()) {
            if (freqT[ch - 'a'] == 0) continue;
            targetCopies = Math.min(
                targetCopies,
                freqS[ch - 'a'] / freqT[ch - 'a']
            );
        }
        return targetCopies;
    }
    public static void main(String[] args) {
        Log.info(rearrangeCharacters("ilovecodingonleetcode", "code" )); // 2
        Log.info(rearrangeCharacters("abcba"                , "abc"  )); // 1
        Log.info(rearrangeCharacters("abbaccaddaeea"        , "aaaaa")); // 1
        Log.info(rearrangeCharacters("abc"                  , "abcd" )); // 0
    }
}
