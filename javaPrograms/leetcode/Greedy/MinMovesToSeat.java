/**
   ...
   You are also given the array students of length n, where students[j] is the
...
class MinMovesToSeat {
    ...
    private static int minMovesToSeat(int[] seats, int[] students) {
        ...
        for (int n : seats) differences[n-1]++;
        for (int n : students) differences[n-1]--;
        int moves = 0;
        int unmatched = 0;
    ...
    public static void main(String[]args) {
        Log.info(minMovesToSeat(new int[]{3,1,5}, new int[]{2,7,4}));     // 4
        Log.info(minMovesToSeat(new int[]{4,1,5,9}, new int[]{1,3,2,6})); // 7
        Log.info(minMovesToSeat(new int[]{2,2,6,6}, new int[]{1,3,2,6})); // 4
    }
}
/**
    private static int minMovesToSeat(int[] seats, int[] students) {
