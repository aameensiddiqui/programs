...
class CountTheNumberOfCompleteComponents {
    ...
    private static void dfs(int x, List<Integer>[] A, boolean[] vis) {
        V++;
        E += A[x].size();
        vis[x] = true;
        for (int state : A[x]) {
            if (!vis[state]) dfs(state, A, vis);
        }
    }
    private static int countCompleteComponents(int n, int[][] edges) {
        List<Integer>[] A = new ArrayList[n];
        Arrays.setAll(A, a -> new ArrayList<>());
    ...
    public static void main(String[] args) {
        log.info(countCompleteComponents(6, new int[][] {{0,1},{0,2},{1,2},{3,4}}));
        log.info(countCompleteComponents(6, new int[][] {{0,1},{0,2},{1,2},{3,4},{3,5}}));
    }
}
