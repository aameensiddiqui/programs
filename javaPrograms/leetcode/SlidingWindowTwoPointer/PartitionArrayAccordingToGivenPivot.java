/**
   ...
   the new position of the jth element. If i < j and both elements are smaller (or larger) than pivot,
   then pi < pj.
...
class PartitionArrayAccordingToGivenPivot {
    ...
    public static void main(String[] args) {
        Log.info(pivotArray(new int[] {9,12,5,10,14,3,10}, 10));
        Log.info(pivotArray(new int[] {-3,4,3,2}, 2));
    }
}
/**
    private static int[] pivotArray(int[] nums, int pivot) {
        ...
        for (int i = 0; i < lt.size() && j < n; ++i, ++j)
            nums[j] = lt.get(i);
        for (int i = 0; i < eq.size() && j < n; ++i, ++j)
            nums[j] = eq.get(i);
        for (int i = 0; i < gt.size() && j < n; ++i, ++j)
            nums[j] = gt.get(i);
        return nums;
    }
*/
